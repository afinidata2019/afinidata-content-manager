...
class GetSessionFieldView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not field.exists():
                return JsonResponse(dict(set_attributes=dict(request_status='error', request_error='Field in position: {0} does not exists.'.format(position))))
            ...
            if field.field_type == 'consume_service':
                ...
                if is_safe_url(service_url, allowed_hosts={'core.afinidata.com',
                                                        ...
                                                        'core.artificial.monster',
                                                        'contentmanager.artificial.monster',
                                                        'program.artificial.monster'
                                                        }, require_https=True):
                    service_params = {}
                ...
                for m in field.message_set.all():
                    ...
                    if field.field_type == 'one_time_notification':
                        messages[-1]['OTN'] = True
                        messages[-1]['last'] = fields.last().position == field.position
                ...
                for condition in field.condition_set.all():
                    ...
                    if attribute_sequence.exists() and condition.attribute == attribute_sequence.first():
                        if condition.condition not in ('is_not_set', 'is_set'):
                            condition.condition = 'is_set' if condition.condition in ('equal', 'in') else 'is_not_set'
